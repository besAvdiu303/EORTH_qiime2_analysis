...
echo "Step 1 - Setting directories for analysis..."
...
echo "Step 1 - Directories set successfully."

# -----------------------------------------------------------------------------
# Step 2 - Generate Phylogenetic Tree
# -----------------------------------------------------------------------------
echo "Step 2 - Generating a phylogenetic tree..."
...
echo "Step 2 - Phylogenetic tree generated."
...
# -----------------------------------------------------------------------------
echo "Step 3 - Running core diversity analysis..."
...
echo "Step 3 - Core diversity analysis completed."

# -----------------------------------------------------------------------------
# Step 4 - Perform Alpha Diversity Significance Tests
# -----------------------------------------------------------------------------
echo "Step 4 - Calculating alpha diversity significance..."
...
echo "Step 4 - Alpha diversity significance analysis completed."

# -----------------------------------------------------------------------------
# Step 5 - Perform Beta Diversity Significance Tests
# -----------------------------------------------------------------------------
echo "Step 5 - Calculating beta diversity significance..."
...
echo "Step 5 - Beta diversity significance analysis completed."

# -----------------------------------------------------------------------------
# Step 6 - Generate Alpha Rarefaction Curve
# -----------------------------------------------------------------------------
echo "Step 6 - Generating alpha rarefaction curve..."
...
echo "Step 6 - Alpha rarefaction curve completed."

# -----------------------------------------------------------------------------
# Step 7 - Download and Apply Taxonomic Classifier
# -----------------------------------------------------------------------------
echo "Step 7 - Downloading taxonomic classifier..."

FILE="$DATA/../gg-13-8-99-515-806-nb-classifier.qza"

if [ ! -f "$FILE" ]; then
  wget -O "$FILE" 'https://docs.qiime2.org/2024.10/data/tutorials/moving-pictures-usage/gg-13-8-99-515-806-nb-classifier.qza'
else
  echo "File already exists at $FILE"
fi

echo "Step 7 - Taxonomic classifier downloaded."

echo "Step 8 - Performing taxonomic classification..."
...
echo "Step 8 - Taxonomic classification completed."

# -----------------------------------------------------------------------------
# Step 9 - Filter Samples for Differential Abundance Analysis
# -----------------------------------------------------------------------------
echo "Step 9 - Filtering samples for differential abundance analysis..."

qiime feature-table filter-samples \
  --i-table $TABLE \
  --m-metadata-file $METADATA \
  --p-where "[$METADATA_COLUMN] = '$SAMPLE_TYPE' AND [$METADATA_COLUMN3] IN ('$REF_LEVEL','$ANALYSIS_LEVEL')" \
  --o-filtered-table 04-differential_abundance/${TAG}_gum-table.qza

echo "Step 9 - Sample filtering completed."

# -----------------------------------------------------------------------------
# Step 10 - Run ANCOM-BC for Differential Abundance Analysis
# -----------------------------------------------------------------------------
echo "Step 10 - Performing differential abundance analysis using ANCOM-BC..."
...
echo "Step 10 - Differential abundance analysis completed."

# -----------------------------------------------------------------------------
# Step 11 - Collapse Taxonomy and Perform ANCOM-BC on Collapsed Data
# -----------------------------------------------------------------------------
echo "Step 11 - Collapsing taxonomy and performing ANCOM-BC analysis..."

qiime taxa collapse \
  --i-table 04-differential_abundance/${TAG}_gum-table.qza \
  --i-taxonomy 03-taxonomy/${TAG}_taxonomy.qza \
  --p-level $TAXA_COLLAPSE_LEVEL \
  --o-collapsed-table 03-taxonomy/${TAG}_gut-table-l${TAXA_COLLAPSE_LEVEL}.qza

qiime composition ancombc \
  --i-table 03-taxonomy/${TAG}_gut-table-l${TAXA_COLLAPSE_LEVEL}.qza \
  --m-metadata-file $METADATA \
  --p-formula $METADATA_COLUMN3 \
  --p-reference-levels $METADATA_COLUMN3::$REF_LEVEL \
...
echo "Step 11 - Taxonomic collapse and differential abundance analysis completed."
echo "Analysis pipeline completed successfully!"

# -----------------------------------------------------------------------------
# Step 12 - Run aldex2 plug-in for generating differentials.qza file needed in R
# -----------------------------------------------------------------------------
